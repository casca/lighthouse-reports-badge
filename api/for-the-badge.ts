...
export default async (req: NowRequest, res: NowResponse) => {
  ...
  try {
    ...
    const percentages: { [key: string]: number } = metrics.reduce(
      (r, m) => ({
        ...r,
        [camelCase(m)]: +(categories && categories[m] && categories[m].score),
      }),
      {}
    );
    ...
    const percentagesStr: { [key: string]: string } = Object.entries(
      percentages
    ).reduce((pStr, [m, p]) => ({ ...pStr, [m]: formatPercentage(p) }), {});
    percentagesStr.average = formatPercentage(
      metrics.reduce((s, m) => s + percentages[camelCase(m)], 0) /
        metrics.length
    );
    percentagesStr.averageWithoutPwa = formatPercentage(
      (percentages.accessibility +
        percentages.bestPractices +
        percentages.seo) /
        3
    );
